...
namespace JDE_Scanner_Desktop.Models
{
    public class AreasKeeper
    {
        ...
        private async void _Remomve(int id)
        {
            ...
            {
                string url = Secrets.ApiAddress + "DeleteArea?token=" + Secrets.TenantToken + "&id={0}&UserId={1}";
        ...
        public async Task Refresh()
        {
            ...
            using (var client = new HttpClient())
            {
                string url = Secrets.ApiAddress + "GetAreas?token=" + Secrets.TenantToken;
        ...
        public async Task<bool> GetMore(int page)
        {
            ...
            using (var client = new HttpClient())
            {
                string url = Secrets.ApiAddress + "GetAreas?token=" + Secrets.TenantToken + "&page=" + page;
                using (var response = await client.GetAsync(new Uri(url)))
                {
                    if (response.IsSuccessStatusCode)
                    {
